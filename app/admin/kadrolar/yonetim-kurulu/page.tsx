...
import { Users, Trash2, Plus, Loader2, User, Briefcase, ArrowLeft, Edit, X, GripVertical, Star } from 'lucide-react';
...
interface BoardMember {
  ...
  title: string;
  image: string;
  type: 'gold' | 'silver'; // gold: Başkan (Altın), silver: Üye (Gümüş)
  order: number;
}

export default function YonetimKuruluYonetimi() {
    ...
    try {
      ...
      members.forEach((item, index) => {
        ...
        if (item.order !== newOrder) {
            const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'board-members', item.id);
  ...
  const handleEditClick = (item: BoardMember) => {
    setName(item.name);
    setTitle(item.title);
    setImage(item.image);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'board-members', editingId);
        await updateDoc(docRef, docData);
      } else {
        await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'board-members'), docData);
      }
      resetForm();
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'board-members', id));
    ...
    <div className="min-h-screen bg-[#0f172a] font-sans text-gray-100 p-6 md:p-12">
      ...
      <div className="max-w-6xl mx-auto mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Yönetim kurulu üyelerini buradan ekleyip düzenleyebilirsiniz.</p>
        </div>
              ...
              <div>
                ...
                <div className="relative">
                    ...
                    >
                        ...
                        <option value="Başkan Vekili">Başkan Vekili</option>
            ...
            {members.length === 0 ? (
              ...
              members.map((item, index) => (
                ...
                >
                  ...
                  {/* Avatar */}
                  <div className={`w-12 h-12 rounded-full overflow-hidden border-2 shrink-0 ${item.type === 'gold' ? 'border-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.3)]' : 'border-slate-500'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-white font-bold text-base mb-0.5 flex items-center gap-2">
                        {item.name}
                        {item.type === 'gold' && <Star size={14} className="text-amber-500 fill-amber-500" />}
                    </h4>
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      ...
                      className={`p-2 rounded-lg transition-colors ${editingId === item.id ? 'bg-amber-500 text-white' : 'text-slate-500 hover:text-amber-400 hover:bg-amber-500/10'}`}
                    >
                      <Edit size={18} />
                    </button>
                    <button 
